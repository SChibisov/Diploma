...
def _cart_from(cart_id, user_mail, product_id, product_count):
    cart = Cart()
    cart.id = cart_id
    cart.user_mail = user_mail
...
class CartRouter(Resource):
    ...
    def put(self, user_id=None):
        ...
        if len(carts) == 0:
            next_cart_id = CartRepository.get_next_cart_id()
            current_cart = _cart_from(next_cart_id, user.email, product_id, product_cnt)
            is_new_cart = True
        else:
            for c in carts:
                if c.product_id == product_id and user.email == c.user_mail:
                    current_cart = c
                    break
